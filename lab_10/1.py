...
def add_data():
    ...
    for row in rows:
        cur.execute(f"INSERT INTO phonebook (name, number) VALUES ('{row[0]}', '{row[1]}')")
...
def update_data():
    initial_name   = input("name of contact: ")
    initial_number = input("number of contact: ")
    if "--" in initial_name or "--" in initial_number:      # preventing SQL-injection
        print("error")
        return

    cur.execute(f"SELECT * FROM phonebook WHERE name = '{initial_name}' AND number = '{initial_number}'")
    if cur.rowcount == 0:
    ...
    cur.execute(f"UPDATE phonebook SET {mode} = '{new_info}' WHERE name = '{initial_name}' AND number = '{initial_number}'")
...
def query_data():
    ...
    cur.execute(f"SELECT * FROM phonebook WHERE {query_dict[query_mode]}")
...
def delete_data():
    ...
    cur.execute(f"DELETE FROM phonebook WHERE {mode} = '{info}'")
